...
#
# Se nenhum diretorio for passado, cria/atualiza em ./sdr-agent
# na pasta onde o script for executado. É seguro rodar de novo:
# os arquivos existentes são sobrescritos com o conteúdo atual
# desta parte (nada é apagado fora do que este script gerencia).

set -euo pipefail

TARGET_DIR="${1:-sdr-agent}"
echo "==> Gerando projeto em: $TARGET_DIR"
...
echo "  - .env.example"
cat > "$TARGET_DIR/.env.example" <<'SDR_PART1_EOF'
# === OpenRouter ===
...
echo "  - .gitignore"
cat > "$TARGET_DIR/.gitignore" <<'SDR_PART1_EOF'
.env
__pycache__/
*.pyc
.pytest_cache/
...
echo "  - .python-version"
cat > "$TARGET_DIR/.python-version" <<'SDR_PART1_EOF'
3.12
SDR_PART1_EOF

echo "  - README.md"
cat > "$TARGET_DIR/README.md" <<'SDR_PART1_EOF'
...
echo "  - pyproject.toml"
cat > "$TARGET_DIR/pyproject.toml" <<'SDR_PART1_EOF'
[project]
name = "sdr-agent"
version = "0.1.0"
...
echo "  - uv.lock"
cat > "$TARGET_DIR/uv.lock" <<'SDR_PART1_EOF'
version = 1
revision = 3
requires-python = ">=3.12"
resolution-markers = [
...
cat > "$TARGET_DIR/app/__init__.py" <<'SDR_PART1_EOF'
SDR_PART1_EOF
...
cat > "$TARGET_DIR/app/agents/_guardrails.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/app/agents/escalate.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/app/agents/knowledge.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/app/agents/objection.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/app/agents/orchestrator.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/app/agents/qualification.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/app/agents/scheduling.py" <<'SDR_PART1_EOF'
"""
...
echo "  - app/main.py"
cat > "$TARGET_DIR/app/main.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/litellm_proxy/config.yaml" <<'SDR_PART1_EOF'
...
litellm_settings:
  ...
  # TODO Parte 5: adicionar "langfuse" aqui para logging de custo/latência
  # nativo do proxy, complementando o tracing feito no lado do ADK.
  success_callback: []
  failure_callback: []
...
echo "  - litellm_proxy/docker-compose.yml"
cat > "$TARGET_DIR/litellm_proxy/docker-compose.yml" <<'SDR_PART1_EOF'
services:
  litellm-proxy:
...
cat > "$TARGET_DIR/tests/__init__.py" <<'SDR_PART1_EOF'
SDR_PART1_EOF

echo "  - tests/test_guardrails.py"
cat > "$TARGET_DIR/tests/test_guardrails.py" <<'SDR_PART1_EOF'
"""
...
cat > "$TARGET_DIR/tests/test_smoke.py" <<'SDR_PART1_EOF'
"""
