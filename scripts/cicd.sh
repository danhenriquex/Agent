...
#   - Dockerfile (build em 2 estagios via uv) e litellm_proxy/Dockerfile
#   - .gitlab-ci.yml (lint/test sempre; build/deploy GCP condicionados
#     a variaveis de CI/CD configuradas)
...
#
# Uso:
#   bash cicd_setup.sh [diretorio-do-projeto]
#
# É seguro rodar de novo: sobrescreve só os arquivos listados acima
# com o conteúdo atual desta trilha.
...
echo "==> Adicionando camada de CI/CD em: $TARGET_DIR"
...
echo "  - app/api.py"
cat > "$TARGET_DIR/app/api.py" <<'SDR_CICD_EOF'
"""
...
cat > "$TARGET_DIR/tests/test_api.py" <<'SDR_CICD_EOF'
"""
...
echo "  - .dockerignore"
cat > "$TARGET_DIR/.dockerignore" <<'SDR_CICD_EOF'
.venv/
venv/
__pycache__/
*.pyc
...
echo "  - litellm_proxy/Dockerfile"
cat > "$TARGET_DIR/litellm_proxy/Dockerfile" <<'SDR_CICD_EOF'
# litellm_proxy/Dockerfile
#
# O docker-compose local (uso em dev) monta config.yaml como volume.
# O Cloud Run não suporta volumes montados a partir do host, então essa
# imagem embute o config.yaml diretamente — qualquer mudança de rota-
# mento de modelo exige rebuild + redeploy desta imagem, não só editar
# um arquivo no servidor. Isso é aceitável aqui porque o roteamento de
# modelo é configuração versionada em código, não segredo de runtime.
...
deploy_sdr_bot_api:
  ...
  script:
    ...
    - >
      ...
      --set-secrets="LITELLM_PROXY_KEY=litellm-proxy-key:latest"
  environment:
...
echo "  - pyproject.toml"
cat > "$TARGET_DIR/pyproject.toml" <<'SDR_CICD_EOF'
[project]
name = "sdr-agent"
version = "0.1.0"
...
echo "  - uv.lock"
cat > "$TARGET_DIR/uv.lock" <<'SDR_CICD_EOF'
version = 1
revision = 3
requires-python = ">=3.12"
resolution-markers = [
...
echo "  - .python-version"
cat > "$TARGET_DIR/.python-version" <<'SDR_CICD_EOF'
3.12
SDR_CICD_EOF

echo "  - README.md"
cat > "$TARGET_DIR/README.md" <<'SDR_CICD_EOF'
